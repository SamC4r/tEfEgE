...
En cuanto al procesamiento, transcodificación y entrega de video se probaron las tres alternativas mencionadas en \ref{2.2}. Para la solución propia se utilizaron los servicios de Amazon Web Service con las tecnologías descritas en \ref{2.1} pero finalmente, se optó por integrar los servicios de un proveedor debido a los costos elevados de las otras alternativas.

Para la creación de un Producto Mínimo Viable (MVP), se ha decido implementar una página web debido a su facilidad de acceso desde cualquier dispositivo y a la adecuación de los \textit{frameworks} y tecnologías web a los objetivos del proyecto. Por ese motivo, a continuación se presentan las tecnologías utilizadas.
...
Next.Js es una tecnología muy poderosa que ya se analizó en secciones anteriores (\ref{Next.Js backend}). Aquí, se estudió su utilidad para tareas de \textit{backend} pero también se integra bien con tareas de \textit{frontend}. 
...
Su funcionamiento consiste en aplicar directamente en un atributo  \lstinline|class| o \lstinline|className| en HTML propiedades CSS (\textit{utiliy classes)} predefinidas que representan características visuales específicas. Por ejemplo, el siguiente código pone a rojo el color del texto a rojo
